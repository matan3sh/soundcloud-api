...
const TrackPreview = ({ track, setTrack, currTrack, clearTrack, mood }) => {
    ...
    return (
        <div className={`trackSearch__preview ${track?.id === currTrack?.id ? 'active' : ''}`} onClick={() => {
            clearTrack()
            ...
            {mood[0] === 'list' ?
                <>
                    <div>
                        ...
                        <h3>{track.title.length > 25 ? track.title.slice(0, 24) + '...' : track.title}</h3>
                    </div>
                    <p><i className="fas fa-play"></i> {durationFormatter(track.duration)}</p>
                </>
                :
                <div className="trackSearch__preview-tile">
                    <img src={track.artwork_url} alt="play" />
                    <h3>{track.title.length > 25 ? track.title.slice(0, 18) + '...' : track.title}</h3>
                    <p><i className="fas fa-play"></i>  {durationFormatter(track.duration)}</p>
                </div>
            }
